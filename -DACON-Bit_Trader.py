...
# many-to-many 모델로 다시 돌려보기(input: 120, output: 10 정도?), epoch 수 늘리고 batch_size 줄여서 다시 학습해 보기, optimizer을 rmsprop으로 설정해보기,
# train_x, train_y로 학습하는 부분은 생략하고 test_x만으로 학습하고 예측해보기. train_x데이터는 test_x데이터로 학습하고 예측하는 '방식'을 그대로 활용했을 때 실제(train_y)와 얼마나 부합하는지 확인하는 용도로만 활용
# LSTM 레이어 멋사 실습한 프로젝트들 보며 조절해보기
